...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagswitch_led)
    {
        info->event[info->num] = EVENT_switch_led;
        info->num++;
        dataPoints->valueswitch_led = gizStandardDecompressionValue(switch_led_BYTEOFFSET,switch_led_BITOFFSET,switch_led_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagswitch_relay)
    {
        info->event[info->num] = EVENT_switch_relay;
        info->num++;
        dataPoints->valueswitch_relay = gizStandardDecompressionValue(switch_relay_BYTEOFFSET,switch_relay_BITOFFSET,switch_relay_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueswitch_led != cur->valueswitch_led)
    {
        GIZWITS_LOG("valueswitch_led Changed\n");
        ret = 1;
    }
    if(last->valueswitch_relay != cur->valueswitch_relay)
    {
        GIZWITS_LOG("valueswitch_relay Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(switch_led_BYTEOFFSET,switch_led_BITOFFSET,switch_led_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswitch_led);
    gizStandardCompressValue(switch_relay_BYTEOFFSET,switch_relay_BITOFFSET,switch_relay_LEN,(uint8_t *)devStatusPtr,dataPoints->valueswitch_relay);
